...
def is_image_file(filename):
    return any(filename.endswith(extension) for extension in IMG_EXTENSIONS)#any():只要有一个为TRUE就返回TURE[0，空，FALSE]
                                                                            #endwith():判断字符串是不是以指定的后缀名结尾

def dataloader(filepath):

 classes = [d for d in os.listdir(filepath) if os.path.isdir(os.path.join(filepath, d))]#os.path.isdir需要绝对路径，所以需要用os.path.join()合成绝对路径
 ...
 for dd in monkaa_dir:
   for im in os.listdir(monkaa_path+'/'+dd+'/left/'):
    if is_image_file(monkaa_path+'/'+dd+'/left/'+im):
     ...
     all_left_disp.append(monkaa_disp+'/'+dd+'/left/'+im.split(".")[0]+'.pfm')#把标签文件的后缀名改为.pfm
